...
class MVAggregate(nn.Module):
    def __init__(self,  model, agr_type="max", feat_dim=400, lifting_net=nn.Sequential(), cv_feat_dim=0):
        super().__init__()
        self.agr_type = agr_type
        self.cv_feat_dim = cv_feat_dim
        self.fused_dim = feat_dim

        if self.cv_feat_dim > 0:
            self.cv_encoder = nn.Sequential(
                nn.LayerNorm(self.cv_feat_dim),
                nn.Linear(self.cv_feat_dim, feat_dim),
                nn.ReLU(),
                nn.Linear(feat_dim, feat_dim),
                nn.ReLU(),
            )
            self.fused_dim = feat_dim * 2

        self.inter = nn.Sequential(
            nn.LayerNorm(self.fused_dim),
            nn.Linear(self.fused_dim, feat_dim),
            nn.Linear(feat_dim, feat_dim),
        )
    ...
    def forward(self, mvimages, cv_features=None):
        ...
        if self.cv_feat_dim > 0:
            if cv_features is None:
                cv_features = torch.zeros((pooled_view.shape[0], self.cv_feat_dim), device=pooled_view.device, dtype=pooled_view.dtype)
            if len(cv_features.shape) == 1:
                cv_features = cv_features.unsqueeze(0)
            cv_features = cv_features.to(device=pooled_view.device, dtype=pooled_view.dtype)
