...
from cv_foul_detection.classical_model import FEATURE_SETS, feature_dim
...
def main(*args):
    ...
    #       LOADING THE MODEL         #
    ###################################
    cv_feat_dim = feature_dim(cv_feature_set) if cv_features_path else 0
    model = MVNetwork(net_name=pre_model, agr_type=pooling_type, cv_feat_dim=cv_feat_dim).cuda()

    if path_to_model_weights != "":
        path_model = os.path.join(path_to_model_weights)
        load = torch.load(path_model)
        if cv_feat_dim > 0:
            load_compatible_state_dict(model, load['state_dict'])
        else:
    ...
    if only_evaluation == 0:
        ...
        results = require_evaluate()(os.path.join(path, "Test", "annotations.json"), prediction_file)
        print("TEST")
        print(results)
        ...
        results = require_evaluate()(os.path.join(path, "Chall", "annotations.json"), prediction_file)
        print("CHALL")
        print(results)
